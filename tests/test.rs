...
#[test]
fn line_separator_runtime_string() {
    let sep = format!("--{}--\n", "sep");
    let grid = Grid::new(
        vec!["a", "b", "c", "d"],
        GridOptions {
...
fn line_separator_builder() {
    ...
    .with_line_separator("\0");
    assert_eq!("a  b\0", grid.to_string());
}
